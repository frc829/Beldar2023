// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.framework.lighting.animations.singleColor;

import frc.robot.framework.lighting.animations.Animation;

/** Add your docs here. */
public class SingleColorForever extends Animation {

    private final int red;
    private final int green;
    private final int blue;
    private final int white;

    public SingleColorForever(int ledStart, int numLed, int red, int green, int blue, int white) {
        super(ledStart, numLed);
        this.red = red;
        this.green = green;
        this.blue = blue;
        this.white = white;
    }

    public SingleColorForever(int ledStart, int numLed, int red, int green, int blue) {
        super(ledStart, numLed);
        this.red = red;
        this.green = green;
        this.blue = blue;
        this.white = 0;
    }

    public int getRed(){
        return red;
    }

    public int getGreen(){
        return green;
    }

    public int getBlue(){
        return blue;
    }

    public int getWhite(){
        return white;
    }

}
